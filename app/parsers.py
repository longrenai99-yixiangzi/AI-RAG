...
def _cell_text(value: object) -> str:
    ...
    if text.upper() in {"#VALUE!", "#DIV/0!", "#REF!", "#NAME?", "#N/A", "#NUM!", "#NULL!"}:
        return ""
    return text
...
def _parse_xlsx(path: Path, document_id: str) -> list[SourceBlock]:
    ...
    try:
        for sheet in workbook.worksheets:
            if sheet.title in EXCLUDED_XLSX_SHEETS:
                continue
            ...
            for row_number, row in enumerate(sheet.iter_rows(values_only=True), start=1):
                ...
                if not any(values):
                    empty_streak += 1
                    if headers is not None and empty_streak >= 200:
                        break
                    continue
                empty_streak = 0
                if headers is None:
