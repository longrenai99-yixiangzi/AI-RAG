...
def _build_compatibility_samples() -> list[dict[str, Any]]:
    ...
    for item in approved:
        ...
        sample = {
            ...
            "question_id": qid,
            "knowledge_root_id": "Root-001" if str(item["gold_primary_source"]).startswith(r"[LOCAL_PATH_REDACTED]") else "Root-002",
...
def _report(metrics: dict[str, Any], replay: list[dict[str, Any]], lineage: dict[str, Any], elapsed: float) -> str:
    return "\n".join([
        ...
        "## 1. 执行边界", "",
        "- Root-001只读扫描：`[LOCAL_PATH_REDACTED]。", "- Root-002只使用已有020A Owner Approved Artifact兼容性样本，治理继续为`PENDING_APPROVAL`。", "- 未修改正式8000、Retriever、BM25、Dense、RRF、Reranker、Router、Scope Guard、Preflight、Fact Path或Answer Engine。", "- 未重建正式Qdrant，未生成正式Collection Embedding，Provider HTTP Requests=0。", "",
        "## 2. Root-001结构指标", "", "| 指标 | 值 |", "|---|---:|",
