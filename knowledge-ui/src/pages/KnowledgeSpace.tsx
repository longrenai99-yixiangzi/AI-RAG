...
function TechnicalConstructionSpace({ sample }: { sample: NonNullable<ReturnType<typeof getKnowledgeNodeSample>> }) {
  ...
  return <section className="sample-space">
    ...
    <SpaceSection title="项目案例">{sample.cases.length ? <div className="case-grid">{sample.cases.map((item) => <article key={item.id}><span>Case · Mock</span><h3>{item.name}</h3><p><b>场景：</b>{item.scenario}</p><p><b>做法：</b>{item.outcome}</p><small>来源：{item.sourceIds.map((sourceId) => sourceById.get(sourceId)?.name).join('；')}</small></article>)}</div> : <EmptySampleSection text="当前节点暂无已抽取的项目案例，未用其他节点案例代替。" />}</SpaceSection>
...
function EmptySampleSection({ text }: { text: string }) { return <p className="sample-empty">{text}</p> }
function SourceItem({ source }: { source: { name: string; type: string; authority: string; sourcePath?: string; location: string; excerpt: string } }) { return <article className="sample-source"><div><span>{source.type} · {source.authority}</span><h3>{source.name}</h3><p>{source.excerpt}</p>{source.sourcePath && <small className="source-path">路径：{source.sourcePath}</small>}</div><small>{source.location}</small></article> }
