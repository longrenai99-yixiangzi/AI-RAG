...
export function KnowledgeMap({ node, children, onSelect }: Props) {
  ...
  return <div className="knowledge-map">
    ...
    <div className="map-children">
      {sample && <div className="sample-node-preview"><span className="map-node-type">KNOWLEDGE NODE PREVIEW</span><b>总体施组 · 已接通真实 Mock 数据</b><p>{sample.cards.length} 张知识卡片 · {sample.sources.length} 条资料来源 · {sample.cases.length} 个项目案例</p><div>{sample.cards.map((card) => <span key={card.id}>{card.title}</span>)}</div><Link to={`/knowledge/${node.id}`}>进入完整知识空间</Link></div>}
