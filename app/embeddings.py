...
class RerankerService:
    ...
    def load(self) -> bool:
        ...
        try:
            ...
            self.tokenizer = AutoTokenizer.from_pretrained(
                str(self.settings.reranker_model_path), local_files_only=True
            )
            self.model = AutoModelForSequenceClassification.from_pretrained(
                str(self.settings.reranker_model_path), local_files_only=True
            )
            self.model.to(self.device).eval()  # type: ignore[attr-defined]
            self.error = None
            return True
        except Exception as error:
            self.error = f"{type(error).__name__}: {error}"
    ...
    def score(self, query: str, passages: Sequence[str]) -> list[float] | None:
        ...
        try:
            ...
            return [float(score) for score in logits.cpu().tolist()]
        except Exception as error:
            self.error = f"{type(error).__name__}: {error}"
            if self.settings.reranker_mode == "on":
