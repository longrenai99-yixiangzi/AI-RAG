...
这是一个本地运行、对 `[LOCAL_PATH_REDACTED]。
...
不会移动、删除、重命名或回写 `[LOCAL_PATH_REDACTED]。运行数据只写入本项目的 `data/`。
...
如果模型已经存在于其他本地缓存目录，可设置 `RAG_MODEL_ROOT` 指向包含 `bge-m3` 和 `bge-reranker-v2-m3` 的目录，避免重复下载。

仅做本地证据检索演示时，可设置 `RAG_ENABLE_LLM=0`；系统不会调用生成模型，只返回带来源的证据并记录知识缺口。

如果 Dense 模型暂时不可用但已有 BM25/SQLite 索引，问答会降级为 BM25 + 证据展示；未配置生成模型时也只返回可核查证据，不会把检索片段伪装成模型结论。
...
候选草稿带有 `formal_write: forbidden`，系统不会自动改写 `[LOCAL_PATH_REDACTED]��知识。正式知识写回仍需人工审核和明确目标。
