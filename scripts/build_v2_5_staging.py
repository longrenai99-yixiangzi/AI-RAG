...
def main() -> int:
    ...
    builder = DocumentIntelligenceV2Builder(Path(r"[LOCAL_PATH_REDACTED]"))
    raw_atomic = []
    for path in SOURCES:
        raw_atomic.extend(build_atomic_evidence(path, Path(r"[LOCAL_PATH_REDACTED]")).get("records") or [])
