...
def main() -> int:
    ...
    provider = BGEM3DenseProvider(MODEL, collection_name="v2_6_2_remediation_embedding", use_fp16=cuda_available, batch_size=64 if cuda_available else 32)
    if base_count == len(chunks):
        fresh = np.empty((0, 1024), dtype=np.float32)
        vectors = existing
    else:
        try:
    ...
    manifest.update({"status": "DEV_REMEDIATION_EMBEDDED_NOT_RELEASED", "embedded_at": now, "embedding_status": "PASS", "dense_embeddings": {"path": str(vector_path), "sha256": _sha(vector_path), "shape": list(vectors.shape), "normalized_new_vectors": True, "model": "BAAI/bge-m3", "base_vectors_reused": base_count, "new_vectors_written": len(fresh), "runtime": "CUDA_FP16" if cuda_available else "CPU_FP32"}, "candidate_hash": hashlib.sha256(candidate_material).hexdigest(), "candidate_revision": "V2.6.2_DEV_SOURCE_COVERAGE", "formal_8000_touched": False, "8010_switch_performed": False})
